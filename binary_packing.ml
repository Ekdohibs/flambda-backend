...
let arch_sixtyfour = true (* Sys.word_size_in_bits = 64 *)
...
let rec last_nonmatch_plus_one ~buf ~min_pos ~pos ~char =
  let pos' = pos - 1 in
  if pos' >= min_pos && ( = ) (Bytes.get buf pos') char
...
let unpack_tail_padded_fixed_string ?(padding = '\x00') ~buf ~pos ~len () =
  ...
  in
  Bytes.to_string (Bytes.sub buf pos (data_end - pos))
;;
...
let pack_tail_padded_fixed_string ?(padding = '\x00') ~buf ~pos ~len s =
  ...
  then
    ...
    Bytes.blit (Bytes.of_string s) 0 buf pos slen;
    if slen < len
    then (
      let diff = len - slen in
      Bytes.fill buf (pos + slen) diff padding))
;;
