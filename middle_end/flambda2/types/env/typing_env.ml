...
module One_level : sig
  ...
  let print ~min_binding_time ppf { scope; level; just_after_level } =
    ...
    then Format.fprintf ppf "@[<hov 0>((scope@ %a)@ %a)@]" Scope.print scope TEL.print level
    else
      Format.fprintf ppf "@[<hov 0>@[<hov 1>((scope@ %a)@ (defined_vars@ %a))@]@ %a@]"
        Scope.print scope
  ...
  let bump_scope t =
    { t with scope = Scope.next t.scope; }
