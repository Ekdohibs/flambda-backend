...
let translate_jump_to_continuation ~dbg_with_inlined:dbg env res apply types
  ...
  then
    ...
    Misc.fatal_errorf "Types (%a) do not match arguments of@ %a"
      (Format.pp_print_list ~pp_sep:Format.pp_print_space (To_cmm_env.print_param_type Printcmm.machtype))
  ...
  let cmm, free_vars, res =
    ...
    then
      ...
      ( C.create_ccatch ~rec_flag:false ~body
          ~handlers:[C.handler ~dbg catch_id (C.actual_params vars) handler is_cold],
        free_vars,
        res )
  in
  let cmm, free_vars = wrap cmm free_vars in
...
and let_cont_rec env res invariant_params conts body =
  ...
  let env, invariant_vars = C.continuation_bound_parameters env invariant_params in
  ...
  let handlers, free_vars =
    Continuation.Map.fold
      (fun k (vars, handler, free_vars_of_handler) (handlers, free_vars) ->
        ...
        C.handler ~dbg id (C.actual_params vars) handler false :: handlers, free_vars)
        ...
        let env, res =
          List.fold_left2
            (fun (env, res) param arg ->
               match[@ocaml.warning "-4"] Flambda_kind.With_subkind.kind (Bound_parameter.kind param) with
               | Rec_info ->
               ...
               | _ ->
                 bind_var_to_simple ~dbg_with_inlined env res
                   ...
                   ~num_normal_occurrences_of_bound_vars:handler_params_occurrences arg
            )
