...
let cse_with_eligible_lhs ~typing_env_at_fork ~cse_at_each_use ~params prev_cse
  ...
  let params_set = List.map params ~f:Bound_parameter.name |> Name.Set.of_list in
      ...
      let find_new_name =
        let rec find_param simple params =
          ...
          | param :: params ->
            ...
            | arg -> if Simple.equal arg simple then Some param else find_param simple params
        in
        ...
        | Non_empty { extra_args; extra_params } ->
          ...
          fun arg ->
            ...
            | None -> find_name arg (Bound_parameters.to_list extra_params) extra_args
            | (Some _) as r -> r
      in
      EP.Map.fold
