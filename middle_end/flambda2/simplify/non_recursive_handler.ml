...
let rename_params t =
  ...
  let params_renaming = Bound_parameters.renaming t.params ~guaranteed_fresh:params in
  let lifted_params, lifted_params_renaming = Lifted_cont_params.rename t.lifted_params in
  (* The order of composition of renamings should not matter, since we expect the params
     and lifted params to be distinct. *)
  let renaming = Renaming.compose ~first:params_renaming ~second:lifted_params_renaming in
