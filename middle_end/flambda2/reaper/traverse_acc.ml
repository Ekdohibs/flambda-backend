...
let add_flows t ~into ~from = Graph.add_flows t.deps ~into ~from
...
let add_argument_dep t ~to_ relation ~base =
  Graph.add_argument_dep t.deps ~to_ relation ~base

let add_parameter_dep t ~base relation ~from =
  Graph.add_parameter_dep t.deps ~base relation ~from

let add_propagate_dep t ~if_used ~into ~from =
  Graph.add_propagate_dep t.deps ~if_used ~into ~from

let add_flows_if_any_source_dep t ~if_any_source ~into ~from =
  Graph.add_flows_if_any_source_dep t.deps ~if_any_source ~into ~from
...
let cond_flows t ~(denv : Env.t) ~from ~into =
  match denv.current_code_id with
  | None -> add_flows t ~from ~into
  | Some code_id ->
    add_propagate_dep t ~if_used:(Code_id_or_name.code_id code_id) ~from ~into
 ...
 * complex parameter, each with parameters for each part of the
...
let make_known_arity_apply_widget t ~(denv : Env.t) ~params ~returns ~exn =
  ...
  in
  cond_flows t ~denv ~from:apply ~into:witness;
  apply
...
let create_unknown_arity_call_witnesses t code_id ~is_tupled ~arity ~params
  ...
  then (
    ...
    in
    add_parameter_dep t ~base:witness (Cofield.param 0) ~from:untuple_var;
    ...
    let rec add_deps params_and_witnesses =
      ...
      | (first, witness) :: rest -> (
        List.iteri
          (fun i arg ->
            add_parameter_dep t ~from:(Code_id_or_name.var arg)
...
let make_unknown_arity_apply_widget t ~(denv : Env.t) ~arity ~params ~returns
  ...
  let apply =
    Code_id_or_name.var (Variable.create "apply" Flambda_kind.rec_info)
  in
  cond_flows t ~denv ~from:apply ~into:(List.hd witnesses);
  apply
...
let record_set_of_closure_deps t =
  List.iter
    (fun { let_bound_name_of_the_closure = name;
      ...
      | code_dep ->
        add_propagate_dep t
          ~into:(Code_id_or_name.var code_dep.my_closure)
          ~from:(Code_id_or_name.name name)
          ~if_used:(Code_id_or_name.code_id code_id);
